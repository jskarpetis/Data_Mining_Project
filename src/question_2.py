...
def find_best_clusters(number_of_clusters, scaled_dataset):
    ...
    for k in range(1, number_of_clusters):
        inertia = kmeans_alg(n_clusters=k, n_init=10, max_iter=30,
                             init='k-means++', dataset=scaled_dataset)
        sse.append(inertia)

    knee_locator = KneeLocator(
        range(1, number_of_clusters), sse, curve='convex', direction='decreasing')
...
def preproccess_data(dataset):
    ...
    batch_size = 288
    dataframe = dataset.loc[:, ['Day ahead forecast', 'Hour ahead forecast', 'Current demand', 'Solar', 'Wind', 'Geothermal',
                                'Biomass', 'Biogas', 'Small hydro', 'Coal', 'Nuclear', 'Natural gas', 'Large hydro', 'Batteries', 'Imports', 'Other']]
    ...
    for column in columns:
        ...
        mini_batches = [data[k: k + batch_size]
...
def handle_data_trial(day_split_dataset):
    # day_split_dataset = day_split_dataset.loc[:, ['Current demand', 'Solar', 'Wind', 'Geothermal', 'Biomass',
    #                                               'Biogas', 'Small hydro', 'Coal', 'Nuclear', 'Natural gas', 'Large hydro', 'Batteries', 'Imports', 'Other']]
...
if __name__ == '__main__':
    ...
    inertia, centroids, labels, label = kmeans_alg(
        n_clusters=8, n_init=10, max_iter=30, init='k-means++', dataset=scaled_dataset)
    ...
    for i in numpy.unique(labels):
        plt.scatter(scaled_dataset[label == i, 0],
                    scaled_dataset[label == i, 1], label=i)
